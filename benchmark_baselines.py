...
from experiments.datasets import load_dataset, preprocess_for_baseline, prepare_label_column


METHODS = ("pca_logreg", "pca_svm", "pca_knn", "leiden")
...
def main():
    parser = argparse.ArgumentParser(description="Baseline comparisons for the scGPT PBMC benchmark")
    ...
    raw, label_key, spec = load_dataset(args.dataset)
    labels = prepare_label_column(raw, label_key)
    adata = preprocess_for_baseline(raw, hvg=args.hvg)
    X = adata.obsm["X_pca"]
